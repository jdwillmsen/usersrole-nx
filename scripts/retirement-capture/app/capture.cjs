...
if (!app || !email || !password) {
  console.error(
    'usage: TEST_EMAIL=.. TEST_PASSWORD=.. capture.cjs <app> <out dir>',
  );
  process.exit(2);
}
...
function maskInitScript(keep) {
  const mine = (el) =>
    keep.some((k) => k && (el.textContent || '').includes(k));
  const apply = () => {
    ...
    document.querySelectorAll('.ag-row').forEach((row) => {
      row.style.filter = keepRows.has(row.getAttribute('row-index'))
        ? ''
        : 'blur(7px)';
    });
    document
...
async function newContext(
  browser,
  { flow, viewport = 'desktop', theme, storageState } = {},
) {
  const context = await browser.newContext({
    ...
    ...(flow
      ? {
          recordVideo: {
            dir: path.join(mediaDir, '.raw', flow),
            size: VIEWPORTS[viewport].viewport,
          },
        }
      : {}),
  ...
  await context.addInitScript(
    maskInitScript,
    [email, DISPLAY_NAME].filter(Boolean),
  );
  ...
  context.on('response', async (response) => {
    ...
    if (
      !request.url().startsWith(app.apiBase) ||
    ...
    apiLog.push({
      ...
      request: {
        ...
        url: request.url(),
        httpVersion: 'HTTP/2',
        headers: Object.entries(await request.allHeaders()).map(
      ...
      response: {
        ...
        httpVersion: 'HTTP/2',
        headers: Object.entries(await response.allHeaders()).map(
          ([name, value]) => ({ name, value }),
        ),
        cookies: [],
        content: {
...
async function clearBanner(page) {
  await page.evaluate(() =>
    document.getElementById('walkthrough-banner')?.remove(),
  );
}
...
async function flow(browser, name, opts, body) {
  ...
  results.push(entry);
  console.log(
    `${name}: ${entry.status}${entry.notes.length ? ' - ' + entry.notes.join('; ') : ''}`,
  );
  return entry;
}
...
async function headedFallback(browser, name, routes) {
  ...
  entry.notes.push(
    'signed-in session unavailable headless (App Check enforcement); capture in the headed session',
  );
  const context = await newContext(browser, {});
  const page = await context.newPage();
  for (const [label, route] of routes) {
    try {
      await go(page, route);
      entry.notes.push(
        `${route} unauthenticated -> ${new URL(page.url()).pathname}`,
      );
      entry.screenshots.push(await shot(page, `${name}-${label}-unauth`));
    } catch (err) {
      entry.notes.push(
        `${route}: ${String(err.message || err).split('\n')[0]}`,
      );
    }
  }
  await context.close();
...
async function main() {
  ...
  if (signedIn) {
    ...
    await flow(
      ...
      async (page, entry) => {
        ...
        for (const [label, route] of [
          ...
          await go(page, route);
          entry.notes.push(
            `/admin/${label} as plain user -> ${new URL(page.url()).pathname}`,
          );
          entry.screenshots.push(
            await shot(page, `flow-04-admin-${label}-as-user`),
          );
        }
    ...
    await flow(
      ...
      async (page, entry) => {
        ...
        entry.notes.push(
          'theme choice persists in localStorage and, when signed in, in Firestore users/<uid>',
        );
      },
    );
    ...
    await flow(
      ...
      async (page, entry) => {
        ...
        entry.notes.push(
          'beforeinstallprompt does not fire in headless Chromium; install banner not observable',
        );
        entry.status = pwa.serviceWorkers.length
          ? 'partial'
          : 'needs-headed-session';
    ...
    results.push({
      ...
      status: 'needs-headed-session',
      notes: [
        'headless sign-in rejected: "Firebase App Check token is invalid" (App Check enforced on Identity Toolkit)',
      ],
      screenshots: [],
      videos: [],
    });
  ...
  for (const provider of app.providers) {
    await flow(browser, `07-oauth-${provider}`, {}, async (page, entry) => {
      ...
      if (popup) {
        ...
        entry.screenshots.push(
          await shot(popup, `flow-07-${provider}-provider-page`).catch(
            () => null,
          ),
        );
        ...
        entry.notes.push(
          'no popup captured (provider may block the headless UA)',
        );
      }
      entry.status = 'needs-headed-session';
      entry.notes.push(
        'completing this sign-in needs a real third-party login (headed)',
      );
    });
  }
  ...
  results.push({
    ...
    status: 'captured',
    notes: [
      `${matrixShots.length} screenshots`,
      signedIn
        ? 'signed-in + signed-out screens'
        : 'signed-out screens only (headless auth blocked)',
    ],
  ...
  const har = {
    log: {
      version: '1.2',
      creator: { name: 'retirement-capture', version: '1' },
      entries: apiLog,
    },
  };
  fs.writeFileSync(
    path.join(outDir, 'api-calls.har.json'),
  ...
  console.log(
    `Signed-in headless: ${signedIn}. API calls logged: ${apiLog.length}`,
  );
}
